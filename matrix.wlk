object nave {
	...
	method sube(alguien){pasajeros.add(alguien)}
	method baja(alguien){pasajeros.remove(alguien)}
	method cantidadPasajeros() {return pasajeros.size()}
	method chocar() {
		pasajeros.forEach({pasajero=>pasajero.saltar()})
		pasajeros.clear()
	}
	method estaElElegido(){return pasajeros.any({pasajero => pasajero.esElElegido()})}
	method acelerar(){
		pasajeros.filter({pasajero => not pasajero.esElElegido()})
			.forEach({pasajero=>pasajero.saltar()})
	}
	method pasajeroMasVital() {return pasajeros.max{ pasajero=> pasajero.vitalidad()}}
	method pasajeroMenosVital() {return pasajeros.min{ pasajero=> pasajero.vitalidad()}}
	method estaEquilibrada(){return self.pasajeroMasVital().vitalidad() < self.pasajeroMenosVital().vitalidad()*2  }
}
